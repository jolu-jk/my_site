document.addEventListener("DOMContentLoaded", () => {
  ...
  const questions = [
    ...
    {
      type: "fill-in-the-blanks",
      text: "____ вращается вокруг Земли",
      blanks: 4,
      letters: ["Л", "А", "Н", "У", "С", "Т", "Р", "О"],
      correctLetters: ["Л", "У", "Н", "А"],
    },
    {
      type: "select-words",
      text: "Выберите слова, связанные с космосом.",
      blanks: 0,
      letters: [],
      correctLetters: ["ЗВЕЗДА", "ПЛАНЕТА", "ОРБИТА"],
      words: ["ЗВЕЗДА", "ПЛАНЕТА", "ОРБИТА", "ЛЕС", "РЕКА"],
    },
    {
      type: "select-words",
      text: "Выберите слова, связанные с Луной.",
      blanks: 0,
      letters: [],
      correctLetters: ["СПУТНИК", "КРАТЕР", "ФАЗА"],
      words: ["СПУТНИК", "КРАТЕР", "ФАЗА", "МОРЕ", "ВЕТЕР"],
    },
    {
      type: "select-words",
      text: "Выберите слова, связанные с космическими полётами.",
      blanks: 0,
      letters: [],
      correctLetters: ["РАКЕТА", "ОРБИТА", "КОСМОНАВТ"],
      words: ["РАКЕТА", "ОРБИТА", "КОСМОНАВТ", "ПОЕЗД", "ДОРОГА"],
    },
  ];
      ...
      gameArea.innerHTML = `
            ...
            <div class="letters">
                ${question.letters
                  .map(
                    (letter) =>
                      `<div class="letter" ondblclick="insertLetter(event, '${letter}')">${letter}</div>`
                  )
                  .join("")}
            </div>
        `;
  ...
  function startFlyingWords(words) {
    ...
    words.forEach((word) => {
      ...
      const containerWidth = wordsContainer.offsetWidth;
      ...
      wordElement.addEventListener("click", (e) =>
        checkWordAnswer(e, wordElement, word)
      );
  ...
  function checkWordAnswer(event, wordElement, word) {
    const question = questions[currentQuestionIndex];
    ...
    if (question.correctLetters.includes(word)) {
      ...
      createExplosion(event.clientX, event.clientY, "#4CAF50");
      ...
      score -= 15;
      timeLeft -= 15;
      createExplosion(event.clientX, event.clientY, "#FF6347");
    }
  ...
  function checkAllWordsSelected() {
    const correctWordsSelected = Array.from(
      document.querySelectorAll(".word")
    ).filter((wordElement) => wordElement.classList.contains("correct"));

    if (
      correctWordsSelected.length ===
      questions[currentQuestionIndex].correctLetters.length
    ) {
      answeredQuestions++;
  ...
  function checkAnswer(blank) {
    const question = questions[currentQuestionIndex];
    const index = Array.from(document.querySelectorAll(".blank")).indexOf(
      blank
    );
    ...
    if (insertedLetter === correctLetter) {
      ...
      createExplosion(centerX, centerY, "#4CAF50");
    } else {
      ...
      createExplosion(centerX, centerY, "#FF6347");
    ...
    const filledBlanks = Array.from(allBlanks).filter(
    ...
    if (filledBlanks.length === allBlanks.length) {
      ...
      gameData.answers.push(
        Array.from(allBlanks).map((blank) => blank.textContent)
      );
      gameData.correctAnswers.push(question.correctLetters);
      gameData.questions.push(question.text);
    ...
    const gameHistory =
      JSON.parse(
        localStorage.getItem(
          "gameHistory_" + localStorage.getItem("gameUsername")
        )
      ) || [];
...
function createExplosion(x, y, color) {
  ...
  for (let i = 0; i < particleCount; i++) {
    ...
    const velocity = Math.random() * 100 + 70;
